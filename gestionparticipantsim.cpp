#include "gestionparticipantsim.h"
...
GestionParticipantSim::GestionParticipantSim()
{
...
GestionParticipantSim::GestionParticipantSim(DatabaseManager *db)
{
    m_db = db;
    ui.setupUi(this);
...
void GestionParticipantSim::init()
{
...
void GestionParticipantSim::createTableView()
{
    // Creation du model
...
void GestionParticipantSim::createMenuBar()
{
    QAction *quitAction = new QAction(tr("&Quitter"), this);
    connect(quitAction, &QAction::triggered, this, &GestionParticipantSim::close);
}

void GestionParticipantSim::showError(const QSqlError &err)
{
...
void GestionParticipantSim::on_suprButton_clicked()
{
...
void GestionParticipantSim::on_updateButton_clicked()
{
    QModelIndexList selectedIndexes = ui.participantTable->selectionModel()->selectedIndexes();
...
void GestionParticipantSim::serialReceived()
{
...
void GestionParticipantSim::razGetData(QByteArray data)
{
    qDebug() << data;
    qDebug() << "getter";
}

void GestionParticipantSim::finishedRead()
{
    if(!ui.razButton->isEnabled())
    {
        ui.razButton->setEnabled(true);
...
void GestionParticipantSim::on_razButton_clicked()
{
    //Recuperer toutes les valeurs situes dans le tableau et récuperer seulement la ligne selectionner pour pouvoir récuperer les valeur de cette ligne.
    QModelIndexList selectedIndexes = ui.participantTable->selectionModel()->selectedIndexes();
    qDebug() << selectedIndexes[0].data(); //Afficher l'id du récuperation de l'id du participant (Test debug pour afficher l'id supprimer)
